...
const SUPABASE_URL = 'https://ttgpuccfyykwguhngemd.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR0Z3B1Y2NmeXlrd2d1aG5nZW1kIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjIwMzAzNjUsImV4cCI6MjA3NzYwNjM2NX0.5NlKgh9srqllT9bIjpKrAhdgFGt_-XHNyCgssmAOHrE'
...
function getUserRecipeCount() {
    return parseInt(localStorage.getItem("text2potai_recipe_count") || "0", 10);
}

function incrementUserRecipeCount() {
    const count = getUserRecipeCount() + 1;
    localStorage.setItem("text2potai_recipe_count", count);
    return count;
}
...
console.log('🥘 Text2PotAI initialized with user:', this.userID, 'db uuid:', this.userUUID);
    }
...
generateUserID() {
    let userID = localStorage.getItem('text2potai_user_id');
    if (!userID) {
        userID = 'user_' + Date.now() + '_' + Math.random().toString(36).slice(2, 11);
        localStorage.setItem('text2potai_user_id', userID);
    }
    return userID;
}
...
async handleRecipeSearch() {
    ...
    const closeBtn = modal.querySelector("#close-upgrade");
    ...
    upgradeBtn.addEventListener("click", async () => {
        try {
            ...
            const res = await fetch("https://ttgpuccfyykwguhngemd.supabase.co/functions/v1/create-checkout", {
                method: "POST",
                headers: {
            ...
            window.location.href = checkout_url; // redirect to payment page
        } catch (err) {
    ...
    loadFavoritesFromLocal() {
        try {
            const saved = localStorage.getItem("text2potai_favorites");
    ...
    saveFavoritesToLocal() {
        try {
            localStorage.setItem("text2potai_favorites", JSON.stringify(this.favorites));
        } catch (error) {
    ...
    async callSupabaseEdgeFunction(ingredients) {
        const response = await fetch('https://ttgpuccfyykwguhngemd.supabase.co/functions/v1/get-recipes', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`
